...
13. [Criar Usuário Bind para samba4-admin](#13-criar-usuário-bind-para-samba4-admin)
14. [Criar Grupos de Acesso para samba4-admin](#14-criar-grupos-de-acesso-para-samba4-admin)
...
### 12.4 Configurar LDAP para aceitar conexões simples (necessário para samba4-admin)
...
## 13. Criar Usuário Bind para samba4-admin

O `samba4-admin` usa uma conta de serviço dedicada para se conectar ao AD via LDAP. **Não use o Administrator** para isso.

### 13.1 Criar a conta de serviço

```bash
sudo samba-tool user create samba4admin \
    --description="Bind user for samba4-admin web panel" \
...
Este DN será usado no `config.toml` do `samba4-admin`:
